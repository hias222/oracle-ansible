...
#18.06.21 - SF - refactor und Bereinigung
#17.06.21 - MFU - remove service_name - not neede with GI
#17.06.21 - MFU - OMF für Control Files, SGA am Ende erhöhen - Parameter momentan 16G
#27.05.21 - SF - Alles spezifische fuer die 2. Clusterseite entfernt

fct_usage()
{
echo -e "
cr_cdb_single_asm.bash <DB_NAME> <DB_UNIQUE_NAME>

Usage:
\t<DB_NAME> = name of the new database 
\t<DB_UNIQUE_NAME> = unique name and sid of the new database
"
}
...
DB_UNIQUE_NAME=$2
SINGLE_ASM=0
SINGLE_ASM=$(ps -ef | grep -i "ocssd.bin" | grep -v grep | wc -l)
SCHUL_HOST='l9693022'
...
if [[ $SINGLE_ASM -eq 0 ]];then
 ...
 export ORACLE_SID=${DB_UNIQUE_NAME} 
 LISTENER_ORA_ASM=${CRS_HOME}/network/admin/listener.ora
...
if [[ $SINGLE_ASM -eq 0 ]];then
 ...
 echo "Starting Single ASM installation with:" | tee -a ${LOGFILE}
fi
echo "" >> ${LOGFILE}
...
#####
echo "#
dispatchers=\"(PROTOCOL=TCP) (SERVICE=${DB_UNIQUE_NAME}XDB)\"
...
nls_territory=\"GERMANY\"
nls_length_semantics=\"CHAR\"
audit_file_dest=\"/orabase/admin/${DB_UNIQUE_NAME}/adump\"
audit_trail=OS
audit_syslog_level=\"local1.warning\"
audit_sys_operations=FALSE
remote_login_passwordfile=EXCLUSIVE
...
log_archive_format=%t_%s_%r.arch
target_pdbs=30
" > init${DB_UNIQUE_NAME}.ora

# control_files=(\"+DG1/${DB_UNIQUE_NAME}/${DB_UNIQUE_NAME}.ctl1\",\"+DG1/${DB_UNIQUE_NAME}/${DB_UNIQUE_NAME}.ctl2\")
...
shutdown abort;
--
startup nomount pfile="init${DB_UNIQUE_NAME}.ora";
--
...
${file6},
${file7}
LOGFILE GROUP 01  SIZE ${LOGFILE_SIZE},
GROUP 02  SIZE ${LOGFILE_SIZE},
GROUP 03  SIZE ${LOGFILE_SIZE} RESETLOGS;
--
...
NEW_CONTROLFILE_1=`echo "select name from v\\$controlfile where rownum=1 ;" | sqlplus -s / as sysdba | sed "/^$/d" | grep -v -- '--' | grep -vi 'NAME'`
NEW_CONTROLFILE_2=`echo "select name from v\\$controlfile minus select name from v\\$controlfile where rownum=1;" | sqlplus -s / as sysdba | sed "/^$/d" | grep -v -- '--' | grep -vi 'NAME'`

NEW_PARAMETER=$(echo control_files=\(\"${NEW_CONTROLFILE_1}\",\"${NEW_CONTROLFILE_2}\"\))
NEW_ENTRY=$(echo ${NEW_PARAMETER//\//\\/})

echo "changing controlfile to $NEW_PARAMETER " | tee -a ${LOGFILE}

sed -i '/control_files/s/.*/'${NEW_ENTRY}'/' ${WORK_DIR}/init${DB_UNIQUE_NAME}.ora
...
if [[ $SINGLE_ASM -eq 0 ]];then
${ORACLE_HOME}/bin/sqlplus -L -S / as sysdba << EOSQL >> ${LOGFILE} 2>&1
create spfile='${ORACLE_HOME}/dbs/spfile${DB_UNIQUE_NAME}.ora' FROM pfile='${WORK_DIR}/init${DB_UNIQUE_NAME}.ora';
EOSQL
else
${ORACLE_HOME}/bin/sqlplus -L -S / as sysdba << EOSQL >> ${LOGFILE} 2>&1
create spfile='+DG1/${DB_UNIQUE_NAME}/spfile${DB_UNIQUE_NAME}' from pfile='${WORK_DIR}/init${DB_UNIQUE_NAME}.ora';
EOSQL
echo "###[STEP]### create crs resources & pw File" | tee -a ${LOGFILE}
${ORACLE_HOME}/bin/srvctl add database -db ${DB_UNIQUE_NAME} -o ${ORACLE_HOME} -spfile +DG1/${DB_UNIQUE_NAME}/spfile${DB_UNIQUE_NAME} -dbname ${DB_NAME} >> ${LOGFILE} 2>&1
...
#####
# turn off not needed
if [[ $SINGLE_ASM -ge 0 ]];then
echo "###[STEP]### enabling RAC" | tee -a ${LOGFILE}
${ORACLE_HOME}/bin/sqlplus -L -S / as sysdba <<EOSQL >> ${LOGFILE} 2>&1
--alter system set remote_listener='${CLUSTER_NAME}.${DOMAIN}:57575' scope=both sid='*';
alter system set cluster_database=false scope=spfile sid='*';
alter system set instance_number=1 scope=spfile sid='${DB_NAME}';
alter system set thread=1 scope=spfile sid='${DB_NAME}';
alter system set undo_tablespace='UNDOTBS1' scope=spfile sid='${DB_NAME}';
alter system set instance_name='${DB_NAME}' scope=spfile sid='${DB_NAME}';
alter system set local_listener='(ADDRESS=(PROTOCOL=TCP)(HOST=${HOST}-vip.${DOMAIN})(PORT=57575))' scope=spfile sid='${DB_NAME}';
/
select comp_id, status from dba_registry;
...
EXEC DBMS_AUTO_TASK_ADMIN.DISABLE(client_name=>'auto space advisor', operation=>NULL, window_name=>NULL);
EOSQL
${ORACLE_HOME}/bin/srvctl stop database -d ${DB_UNIQUE_NAME} -o abort >> ${LOGFILE} 2>&1
${ORACLE_HOME}/bin/srvctl start database -d ${DB_UNIQUE_NAME} >> ${LOGFILE} 2>&1
echo "" >> ${LOGFILE}
fi
...
ENTRY=$(echo "#Eintrag fuer DG ${DB_UNIQUE_NAME}, $(date '+%d.%m.%Y')

SID_LIST_LISTENER =
  (SID_LIST =
    (SID_DESC =
      (GLOBAL_DBNAME = ${ORACLE_SID}_HV )
      (ORACLE_HOME = ${ORACLE_HOME})
      (SID_NAME = ${ORACLE_SID})
    )
  )
...
grep -i  "${DB_UNIQUE_NAME}[: $=]" ${LISTENER_ORA_ASM} >/dev/null
RC=$?
ENTRY=''

####
# generate tnsnames.ora Entry for SINGLE-ASM
####
echo "###[STEP]### making tnsnames.ora Entry" | tee -a ${LOGFILE}
ENTRY=$(echo "#Eintrag fuer Datenbank ${DB_UNIQUE_NAME}, $(date '+%d.%m.%Y')
${DB_UNIQUE_NAME} =
  (DESCRIPTION =
    (ADDRESS_LIST =
      (ADDRESS = (PROTOCOL = TCP)(HOST = $(uname -n).${DOMAIN})(PORT = 57575))
    )
    (CONNECT_DATA =
      (SERVER = DEDICATED)
...
ENTRY=$(echo "${DB_UNIQUE_NAME}:${ORACLE_HOME}:N")
...
${ORACLE_HOME}/bin/srvctl stop database -d ${DB_UNIQUE_NAME} -o immediate >> ${LOGFILE} 2>&1
${ORACLE_HOME}/bin/srvctl start database -d ${DB_UNIQUE_NAME} >> ${LOGFILE} 2>&1
fi
echo "" >> ${LOGFILE}
...
echo "###[STEP]### prepare for dataguard..." | tee -a ${LOGFILE}

DATAGUARD_SQL_PREPARE=/tmp/prepare_dg.sql

echo "-- prepare for dataguard" > ${DATAGUARD_SQL_PREPARE}

function createStandbyLogfiles() {
  echo "alter database add standby logfile thread 1 group 101 size ${2}" >> ${3};
  echo "alter database add standby logfile thread 1 group 102 size ${2}" >> ${3};
  echo "alter database add standby logfile thread 1 group 103 size ${2}" >> ${3};
  echo "alter database add standby logfile thread 1 group 104 size ${2}" >> ${3};
}

echo "create file ${DATAGUARD_SQL_PREPARE}" >> ${LOGFILE}

createStandbyLogfiles ${LOGFILE_NUMBER_OF_FILES} ${LOGFILE_SIZE} ${DATAGUARD_SQL_PREPARE}

${ORACLE_HOME}/bin/sqlplus -L -S / as sysdba << EOSQL >> ${LOGFILE} 2>&1
@${DATAGUARD_SQL_PREPARE}
--
...
--
ALTER SYSTEM SET DG_BROKER_CONFIG_FILE1 = '+DG1/${DB_UNIQUE_NAME}/DR1.DAT' SCOPE=BOTH;
ALTER SYSTEM SET DG_BROKER_CONFIG_FILE2 = '${ORACLE_HOME}/dbs/${DB_UNIQUE_NAME}_DR2.DAT' SCOPE=BOTH;
--
alter system reset service_names scope=spfile;

-- log_archive_max_processes 20 ok
-- db_writer_processes 8  CPU/8 ??
EOSQL
